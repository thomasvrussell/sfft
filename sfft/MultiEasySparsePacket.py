...
class MultiEasy_SparsePacket:
    def __init__(self, FITS_REF_Queue, FITS_SCI_Queue, FITS_DIFF_Queue=[], FITS_Solution_Queue=[], \
        ...
        ConstPhotRatio=True, MaskSatContam=False, GAIN_KEY='GAIN', SATUR_KEY='ESATUR', DETECT_THRESH=2.0, \
        DETECT_MINAREA=5, DETECT_MAXAREA=0, BoundarySIZE=30, Hough_FRLowerLimit=0.1, BeltHW=0.2, \
        MatchTolFactor=3.0, MAGD_THRESH=0.12, StarExt_iter=4, XY_PriorBan_Queue=[], \
        ...
        * Parameters for Sparse-Flavor SFFT [multiple tasks]
        ...
        -SATUR_KEY ['ESATUR']       # keyword of effective saturation in FITS header (of reference & science), for SExtractor configuration.
        ...
        -CheckPostAnomaly [False]   # the default source selecting (see sfft.Auto_SparsePrep.AutoMask) does not necessarily 
                                    ...
                                    #       in a re-subtraction for refinement.
            ...
            for taskidx in taskidx_tlst:
                ...
                try:
                    with TimeoutAfter(timeout=TIMEOUT_4PREPRO_EACHTASK, exception=TimeoutError):
                        ...
                        SFFTPrepDict = Auto_SparsePrep(FITS_REF=FITS_REF, FITS_SCI=FITS_SCI).AutoMask\
                            (GAIN_KEY=self.GAIN_KEY, SATUR_KEY=self.SATUR_KEY, DETECT_THRESH=self.DETECT_THRESH, \
                            DETECT_MINAREA=self.DETECT_MINAREA, DETECT_MAXAREA=self.DETECT_MAXAREA, \
                            BoundarySIZE=self.BoundarySIZE, Hough_FRLowerLimit=self.Hough_FRLowerLimit, \
                            BeltHW=self.BeltHW, MatchTolFactor=self.MatchTolFactor, MAGD_THRESH=self.MAGD_THRESH, \
